...
public class SqlAssembleUtils {
    ...
    public static BoundSql buildInsertSql(Object entity, Criteria criteria, NameHandler nameHandler) {
        ...
        List<AutoField> entityAutoField = getEntityAutoField(entity, AutoField.UPDATE_FIELD, true);
    ...
    public static BoundSql buildUpdateSql(Object entity, Criteria criteria, NameHandler nameHandler) {
        ...
        List<AutoField> entityAutoField = getEntityAutoField(entity, AutoField.UPDATE_FIELD, false);
        ...
        while (iterator.hasNext()) {
            ...
            if (ArrayUtils.isEmpty(autoField.getValues()) || isEmptyValue(autoField.getValues()[0])) {
                sql.append("NULL");
            } else {
                sql.append("?");
     ...
     */
    private static List<AutoField> getEntityAutoField(Object entity, int operateType, boolean ignoreNull) {
    ...
    private static BoundSql builderWhereSql(List<AutoField> autoFields, NameHandler nameHandler){
    	List<Object> params = new ArrayList<Object>();
    	return builderWhereSql(autoFields, nameHandler, params);
    }
    
    private static BoundSql builderWhereSql(List<AutoField> autoFields, NameHandler nameHandler, List<Object> params) {
        ...
        while (iterator.hasNext()) {
            ...
            if (sql.length() > 0) {
                sql.append(" ");
                if (StringUtils.isNotEmpty(autoField.getSqlOperator())){
                	sql.append(autoField.getSqlOperator()).append(" ");
                }
            }
            
            // 判断是否是子条件
            if (autoField.getName() == null && autoField.getFieldOperator() == null &&
            		autoField.getValues() != null && autoField.getValues().length==1 &&
            		autoField.getValues()[0] instanceof Criteria){
            	// 添加操作
            	sql.append("( ");
            	
            	Criteria subexpr = (Criteria) autoField.getValues()[0];
            	BoundSql subsql = builderWhereSql(subexpr.clone().getAutoFields(), nameHandler, params);
            	sql.append(subsql.getSql());
            	sql.append(" )");
            	continue;
            }
    ...
    public static BoundSql buildDeleteSql(Object entity, Criteria criteria, NameHandler nameHandler) {
        ...
        List<AutoField> entityAutoField = getEntityAutoField(entity, AutoField.WHERE_FIELD, true);
